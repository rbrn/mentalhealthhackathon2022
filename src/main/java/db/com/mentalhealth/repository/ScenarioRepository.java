...
public interface ScenarioRepository extends JpaRepository<Scenario, Long> {
    ...
    @Query(
        value = "select distinct scenario from Scenario scenario left join fetch scenario.category left join fetch scenario.subcategory left join fetch scenario.question",
    ...
    @Query(
        "select distinct scenario from Scenario scenario left join fetch scenario.category left join fetch scenario.subcategory left join fetch scenario.question"
    )
    List<Scenario> findAllWithToOneRelationships();

    @Query(
        "select scenario from Scenario scenario left join fetch scenario.category left join fetch scenario.subcategory left join fetch scenario.question where scenario.id =:id"
    )
